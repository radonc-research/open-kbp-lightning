...
class DosePrediction(pl.LightningModule):
    ...
    def train_dataloader(self):
        ...
        self.data_loader_train = dataloader(self.plan_paths, batch_size=self.hparams.batchsize, trans=['flip', 128, 'crop'])
...
def main(args):
    ...
    name = f'Unet_{args.used_act}_use_resnet_{str(args.use_resnet)}_norm_{args.norm}_{str(args.num_layers)}layer_loss_use_mask_{str(args.use_mask)}'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--validation_data_dir', type=str, default='data/validation-pats')
    parser.add_argument('--test_data_dir', type=str, default='data/test-pats')
    parser.add_argument('--ngpus', type=int, default=1)
    parser.add_argument('--num_layers', type=int, default=2, help='number of layers used of the 3D feature loss. 0=no feature loss.')
    parser.add_argument('--loss_weights', nargs='+',type=int, default=[100, 1, 1, 1, 1], help='weighting of the different feature layers')
    parser.add_argument('--use_mask', type=bool, default=True, help='use mask for feature loss')
    ...
    parser.add_argument('--use_resnet', type=bool, default=True, help='use resnet blocks between down/up convolutions.')
    parser.add_argument('--used_act', type=str, default='Mish', help='Mish|ReLU')
